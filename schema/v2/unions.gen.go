...
//
// RequestPermissionSubject is a tagged union discriminated by the "type" member. Use [NewRequestPermissionSubject]
// or a type switch on [RequestPermissionSubject.Variant] to work with it. The zero value holds no
...
// RequestPermissionSubjectVariants is the set of RequestPermissionSubject variant types. It lists the types
// themselves: a pointer to a variant also has the RequestPermissionSubjectVariant
// methods, but is not one.
type RequestPermissionSubjectVariants interface {
	RequestPermissionSubjectToolCall |
...
func (u RequestPermissionSubject) As[T RequestPermissionSubjectVariants]() (T, bool) {
	v, ok := u.value.(T)
	return v, ok
}
...
func (u RequestPermissionSubject) MarshalJSONTo(enc *jsontext.Encoder) error {
	if u.value == nil {
		return errors.New("RequestPermissionSubject: no variant set; use NewRequestPermissionSubject")
	}
...
// DiffChange is a tagged union discriminated by the "operation" member. Use [NewDiffChange]
// or a type switch on [DiffChange.Variant] to work with it. The zero value holds no
// variant: an omitzero field omits it, and encoding it anywhere else fails.
//
...
// DiffChangeVariants is the set of DiffChange variant types. It lists the types
// themselves: a pointer to a variant also has the DiffChangeVariant
// methods, but is not one.
type DiffChangeVariants interface {
	DiffChangeAdd |
		DiffChangeDelete |
		DiffChangeModify |
		DiffChangeMove |
...
func NewDiffChange[T DiffChangeVariants](v T) DiffChange { return DiffChange{value: v} }
...
// As returns the variant if it is a T, like a type assertion on Variant
// with T checked against the union's variants at compile time.
func (u DiffChange) As[T DiffChangeVariants]() (T, bool) { v, ok := u.value.(T); return v, ok }
...
func (u DiffChange) MarshalJSONTo(enc *jsontext.Encoder) error {
	if u.value == nil {
		return errors.New("DiffChange: no variant set; use NewDiffChange")
	}
	return json.MarshalEncode(enc, u.value)
}
...
// ReplayFrom is a tagged union discriminated by the "type" member. Use [NewReplayFrom]
// or a type switch on [ReplayFrom.Variant] to work with it. The zero value holds no
// variant: an omitzero field omits it, and encoding it anywhere else fails.
//
...
// ReplayFromVariants is the set of ReplayFrom variant types. It lists the types
// themselves: a pointer to a variant also has the ReplayFromVariant
// methods, but is not one.
type ReplayFromVariants interface {
	ReplayFromStart |
		ReplayFromCustom
	ReplayFromVariant
}
...
func NewReplayFrom[T ReplayFromVariants](v T) ReplayFrom { return ReplayFrom{value: v} }
...
// As returns the variant if it is a T, like a type assertion on Variant
// with T checked against the union's variants at compile time.
func (u ReplayFrom) As[T ReplayFromVariants]() (T, bool) { v, ok := u.value.(T); return v, ok }
...
func (u ReplayFrom) MarshalJSONTo(enc *jsontext.Encoder) error {
	if u.value == nil {
		return errors.New("ReplayFrom: no variant set; use NewReplayFrom")
	}
	return json.MarshalEncode(enc, u.value)
}
