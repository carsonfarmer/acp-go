...
func TestUnhandledNotificationsAreIgnored(t *testing.T) {
	...
	agentConn, clientConn := acp2.Pipe(ctx, func(*acp2.AgentSideConnection) acp2.Agent { return bareAgent{} },
	...
	for name, send := range map[string]func() error{
		"document/didOpen": func() error {
			return clientConn.DidOpenDocument(ctx, &acp2.DidOpenDocumentNotification{
		...
		"elicitation/complete": func() error {
			return agentConn.CompleteElicitation(ctx, &acp2.CompleteElicitationNotification{ElicitationID: "elicitation_1"})
		},
