...
type testSession struct{ cwd acpv2.AbsolutePath }
...
type testAgent struct {
	*acpv2.SessionManager[*testSession]
	client acpv2.Client

	cancelled   chan acpv2.SessionID
...
func newTestAgent() *testAgent {
	return &testAgent{
		SessionManager: acpv2.NewSessionManager(
			acpv2.NewMemoryStore[*testSession](),
			func(_ context.Context, params *acpv2.NewSessionRequest) (acpv2.SessionID, *testSession, error) {
				return acpv2.GenerateSessionID(), &testSession{cwd: params.Cwd}, nil
			},
		),
		cancelled:   make(chan acpv2.SessionID, 1),
		mcpNotified: make(chan string, 1),
		initialized: make(chan *acpv2.InitializeRequest, 1),
	}
}

func (a *testAgent) Initialize(_ context.Context, params *acpv2.InitializeRequest) (*acpv2.InitializeResponse, error) {
...
func (a *testAgent) Prompt(ctx context.Context, params *acpv2.PromptRequest) (*acpv2.PromptResponse, error) {
	err := a.client.SessionUpdate(ctx, &acpv2.UpdateSessionNotification{
	...
	return &acpv2.PromptResponse{MessageID: "msg_1"}, nil
}

func (a *testAgent) CancelSession(_ context.Context, params *acpv2.CancelSessionNotification) error {
	a.cancelled <- params.SessionID
	return nil
}

func (a *testAgent) MessageMCP(_ context.Context, params *acpv2.MessageMCPRequest) (*acpv2.MessageMCPResponse, error) {
	result := acpv2.MessageMCPResponse(`{"echo":"` + params.Method + `"}`)
	return &result, nil
}

func (a *testAgent) NotifyMCP(_ context.Context, params *acpv2.MessageMCPNotification) error {
	a.mcpNotified <- params.Method
	return nil
}

type testClient struct {
	updates chan *acpv2.UpdateSessionNotification
}

func newTestClient() *testClient {
	return &testClient{updates: make(chan *acpv2.UpdateSessionNotification, 16)}
}

func (c *testClient) SessionUpdate(_ context.Context, params *acpv2.UpdateSessionNotification) error {
	c.updates <- params
	return nil
}

func (c *testClient) RequestPermission(_ context.Context, params *acpv2.RequestPermissionRequest) (*acpv2.RequestPermissionResponse, error) {
	return &acpv2.RequestPermissionResponse{
...
func TestPromptTurn(t *testing.T) {
	...
	initialized, err := conn.Initialize(ctx, &acpv2.InitializeRequest{
		ProtocolVersion: acpv2.ProtocolVersion,
	...
	if initialized.ProtocolVersion != acpv2.ProtocolVersion || initialized.Info.Name != "test-agent" {
	...
	created, err := conn.NewSession(ctx, &acpv2.NewSessionRequest{Cwd: "/tmp"})
	if err != nil {
		t.Fatalf("NewSession: %v", err)
	}
	result, err := conn.Prompt(ctx, &acpv2.PromptRequest{
		SessionID: created.SessionID,
		Prompt:    []acpv2.ContentBlock{schema.NewContentBlock(schema.ContentBlockText{Text: "hi"})},
	})
	if err != nil {
...
func TestMCPMessageRequestAndNotificationAreSplit(t *testing.T) {
	...
	result, err := conn.MessageMCP(ctx, &acpv2.MessageMCPRequest{ConnectionID: "c1", Method: "tools/list"})
	if err != nil {
	...
	if err := conn.NotifyMCP(ctx, &acpv2.MessageMCPNotification{ConnectionID: "c1", Method: "notifications/initialized"}); err != nil {
...
func TestSessionManagerServesLifecycleMethods(t *testing.T) {
	...
	listed, err := conn.ListSessions(ctx, &acpv2.ListSessionsRequest{})
	if err != nil {
...
func TestCancelSessionReachesTheAgent(t *testing.T) {
	agent := newTestAgent()
	conn, _ := connect(t, agent, newTestClient())
	if err := conn.CancelSession(t.Context(), &acpv2.CancelSessionNotification{SessionID: "session_1"}); err != nil {
...
func TestAgentCallsBackIntoTheClient(t *testing.T) {
	_, agentConn := connect(t, newTestAgent(), newTestClient())
	granted, err := agentConn.RequestPermission(t.Context(), &acpv2.RequestPermissionRequest{
		SessionID: "session_1",
		Title:     "Edit file",
